...
void executePairOfCommands(cmdLine *pCmdLine, int **pipesfd);
...
/*FIXME still need to figure how to handle all commands, for now the executePairOfCommands
will handle each pair: need to fix rederication here and logic in main execute*/
void executePairOfCommands(cmdLine *pCmdLine, int **pipesfd) {
    ...
    int* sinkPipeForfisrtPrg = feedPipe(pipesfd,pCmdLine);
    int* feedPipeForSecondePrg = feedPipe(pipesfd,pCmdLine->next);
    ...
    if (cpid == 0) {   
        ...
        /*---Terminal -FIXME Father Number 1----*/
        close(sinkPipeForfisrtPrg[1]);          /* [> Close unused write end <]*/
